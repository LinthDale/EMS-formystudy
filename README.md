...
│plc-001         │    :5021    │Telegraf    │  ems/   │Telegraf    │        │TimescaleDB table   │
└────────────────┘            └────────────┘ factory  └────────────┘        └────────────────────┘
        ▲
        │ MCP read/write
        │
...
┌────────────────┐
│kc-mqtt-sim     │  MQTT topic: factory/sensor/temp_01（第三方來源、JSON 格式）
└────────────────┘
```

MQTT topic 命名詳見 `doc/adr/ADR-007-mqtt-topic-naming.md`。主規範：`ems/<domain>/{device_id}/measurements`（電表 `ems/devices/...`、工廠 PLC `ems/factory/...`）。第三方 JSON sensor 為例外，沿用上游 topic 由 `kc-ingest` 解析。
...
```bash
# 全部 ems/* 來源（含電表 ems/devices/* 與工廠 PLC ems/factory/*）
docker exec -it ems-mosquitto mosquitto_sub -t 'ems/#' -v

# 第三方 KC JSON sensor（topic 例外）
docker exec -it ems-mosquitto mosquitto_sub -t 'factory/sensor/+' -v
```
...
```text
ems/factory/plc-001/measurements factory_measurements,device_id=plc-001,device_type=plc temperature=28.1,humidity=61.5,pressure=101.3,motor_speed=1450,pump_on=true,valve_open=false 1682000000000000000
```

KC JSON sensor 資料（不同 topic）：

```text
factory/sensor/temp_01 {"temp": 28.1, "hum": 61.5}
```
...
| MQTT `ems/devices/#` 空空 | gateway 連不到 simulator | 看 `docker compose logs gateway` 是否有 connection refused |
| MQTT `ems/factory/#` 空空 | kc-gateway 連不到 kc-modbus-sim | 看 `docker compose logs kc-gateway kc-modbus-sim` |
| MQTT `factory/sensor/#` 空空 | kc-mqtt-sim 未正常送資料 | 看 `docker compose logs kc-mqtt-sim` |
...
├── doc/
│   ├── PRD-架構設計-Guideline.md     # 架構設計權威文件（強制遵循）
│   ├── governance/                   # 規範與設計準則（必讀）
│   │   ├── nfr.md                    #   量化 NFR（Dev/Demo/POC/Prod）
│   │   ├── risk-register.md          #   風險登記簿（13 項）
│   │   └── threat-model.md           #   STRIDE 威脅建模
│   ├── architecture/                 # C4 Context / Container / Data Flow
│   ├── adr/                          # Architecture Decision Records（ADR-001~008）
│   ├── prd/                          # PRD（PRD-0001、PRD-0002）
│   ├── operations/                   # 運維操作
│   │   ├── 操作手冊.md               #   主要操作流程
│   │   ├── 容器速查表.md             #   12 個容器職責、端口、log
│   │   └── network/
│   │       └── Cloudflare_Grafana_Demo_對外公開操作指南.md
│   └── archive/                      # 已被 PRD 取代的歷史文件
│       ├── plan/                     #   原始規劃（EMS 實作計畫、KC 整合 plan）
│       ├── stage_1/README.md         #   Stage 1 進度紀錄
│       └── stage_2/README.md         #   Stage 2 進度紀錄
├── external/
...
| `doc/prd/` | 既有 PRD（PRD-0001 基礎管線、PRD-0002 KC 整合） |
| `doc/architecture/` | C4 Context / Container / Data Flow Mermaid 圖 |
| `doc/adr/` | Architecture Decision Records（ADR-001~008） |
...
| 文件 | 用途 |
|------|------|
| `doc/operations/操作手冊.md` | 主要操作流程，已整合 KC、Grafana、MCP |
| `doc/operations/容器速查表.md` | 12 個容器的端口、職責、log、重建與安全基線 |
| `doc/operations/network/Cloudflare_Grafana_Demo_對外公開操作指南.md` | 使用 Cloudflare Tunnel + Access 對外公開 Grafana demo |

### API 與規劃

| 文件 | 用途 |
|------|------|
| `api/openapi.yml` | PostgREST + Simulator + Grafana API schema |
| `doc/archive/plan/EMS實作計畫.md` | Stage 規劃原始文件（已 PRD 化於 PRD-0001） |
| `doc/archive/plan/kc_integration_plan.md` | KC integration phase 進度（已 PRD 化於 PRD-0002） |
