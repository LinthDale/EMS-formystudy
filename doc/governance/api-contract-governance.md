...
- **Non-breaking（允許在 MINOR）**：新增端點；新增**選填**欄位；新增列舉值（消費端須容忍未知值）；放寬輸入限制。
...
| **contract test** | 由 spec 生 client，對實際服務（或 mock）驗 request/response | 防實作與 spec 漂移 |
| **version check** | 自訂 | 端點 / schema 有改但 `info.version` 未 bump → fail |

> CI 接線屬實作工項（與 PRD-0005 P1 的契約測試同批）；本治理文件定義**規則**，落地時補 CI job。
