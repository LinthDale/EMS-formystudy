...
- **`/ai-feedback` + correction loop（分 3 slice）**：
  ...
  - ⏳ **2c**：相關性 retrieval（同 device/gateway/type 家族/topic prefix 聯集，無筆數上限 FR-331）→ sanitize 注入 prompt + `applied_count` 累加 + 衝突來源接 classifier（FR-332）+ 32KB user_message LRU 斷路（§8.6.5a）；**並補 FR-330 `rerun_classification`（即時過 budget gate 重跑分類）/ `demote_to_candidate`（freeze-override 解凍→candidate→可被 AI 重新分類）兩個 flag 的實際行為（目前 501）**。（FR-344 大量 deactivate alert 仍歸入下方 audit/observability 批次。）
