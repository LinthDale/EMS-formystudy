...
- **MCP server**：device-service 自帶 127.0.0.1:8766，AI 通道僅 `list_low_confidence_candidates` / `get_device_digest` / `classify_with_context`（ADR-012）。**內含「on-demand reclassify 原語」**（取既有裝置近 ≤20 筆原始量測樣本經 OPS pool 讀 measurements §234/§761 → sanitize → 過 budget gate → classify → 寫回），FR-330 `rerun_classification` 與 `classify_with_context` 共用此原語，一併在此批次落地。
