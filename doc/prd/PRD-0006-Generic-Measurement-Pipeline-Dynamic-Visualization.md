...
| 欄位 | 內容 |
|------|------|
| 狀態 | **Draft v2**（2026-06-10；owner review 3 P1 + 4 P2 已全數補正，見最後修訂）|
| 起案日期 | 2026-06-10 |
| 最後修訂 | 2026-06-10 v2：FR-614 補 migration 014 audit CHECK 擴充前置（P1）；FR-605/§7.3/§7.4 web_anon 改 P1 零權限+註冊過濾規則（P1）；FR-601 補 signal_name regex/保留字/value_text≤256/cardinality 軟限（P1）；FR-603 改抽共享 parser 套件非複製（P2）；FR-606 明確 `signal_repo.promote_suggestions()` 新方法（P2）；FR-601 補 time 來源定義（P2）；FR-602 補 Timescale 版本 pin + smoke test（P2）；§11 +R7/R8/R9 |
...
- **FR-601 窄表 `signal_measurements`**〔P1,M〕：`public.signal_measurements(time TIMESTAMPTZ, device_id TEXT, signal_name TEXT, value_num DOUBLE PRECISION, value_bool BOOLEAN, value_text TEXT)`，CHECK「三個 value 欄**恰一非 NULL**」；hypertable（chunk interval 可調，入 tunable-parameters）；索引 `(device_id, signal_name, time DESC)`。
  - **欄位約束（review P1：cardinality / 外洩防護；DB CHECK + ingest 拒收雙層）**：`signal_name` 須符 `^[a-zA-Z0-9_-]{1,64}$`（**沿用 FR-322 既有 regex**；DB CHECK + ingest 驗證，不符→整列拒收+metric）；`signal_name` 禁用保留字 `time`/`device_id`/`signal_name`；`value_text` 上限 **256 字元**（DB CHECK `char_length(value_text)<=256`；ingest 超限→**拒收**該欄+metric，不截斷——截斷=靜默失真）。**cardinality**：單訊息沿用 `MAX_FIELDS=64`；per-device 相異 signal 數為軟限（Grafana 計數監看、>64 告警），不在熱路徑硬擋。
  - **time 來源（review P2，明確定義）**：優先取**來源時戳**（ILP timestamp / JSON `ts`）且 `|ts − now()| ≤ MAX_CLOCK_SKEW`（可調，預設 7 天 = 壓縮窗對齊）；無來源時戳或超窗 → 用 **ingest 收包時間 now()** 並計 fallback metric。注意：現行 `parse_fields` **丟棄 ILP timestamp** —— 共用 parser（FR-603）必須改為曝露它。亂序資料於壓縮窗內照常 INSERT；早於已壓縮 chunk 的寫入成本高，靠 skew 窗抑制。
  - **驗收**：合法 INSERT 成功；違反恰一非 NULL / regex / 長度 → 23514；帶合法 ILP 時戳→time=來源時戳；超窗→time=收包時間且 fallback metric +1。
- **FR-602 day-one columnstore 壓縮**〔P1,M〕：`segmentby = (device_id, signal_name)`、`orderby = time DESC`；壓縮 policy 預設 7 天（可調）。**前置（review P2）**：`docker-compose.yml` 現用 `timescale/timescaledb:latest-pg15` —— **必須先 pin 到具體版本 tag**（壓縮/columnstore 語法依版本而異，latest 使 migration 019 不可重現）；並加 **migration smoke test**（throwaway 容器套 019 → 斷言 hypertable + 壓縮 policy 存在）。**驗收**：`timescaledb_information.jobs` 存在該 policy；壓縮後 chunk 可查；pinned 版本記入 compose + ADR-022。

### Ingest（P1）

- **FR-603 `services/ingest-generic/`**〔P1〕：aiomqtt 訂閱 `ems/+/+/measurements` + `factory/sensor/+`（**client_id 與 device-service 區隔**，雙消費者並存）。**parser 共用而非複製（review P2 改向）**：把 `topic_parser` 的 deny-by-default 准入 + `parse_fields`（ILP/JSON，現在 `discovery.py`）**抽成共享套件**（如 `libs/ems_parsing/`，device-service 與 ingest-generic **import 同一份**，兩容器 build 皆掛入）——複製兩份是未來 bug 源；**parity 測試保留**作回歸防線（鎖共享套件行為，非鎖兩份實作）。共享 parser 同時補：**曝露 ILP timestamp**（FR-601 time 來源需要）、signal_name regex / value_text 長度驗證（FR-601 約束）。型別**決定性**路由（numeric→value_num、bool→value_bool、其餘→value_text）；批次 INSERT。**accept-all 哲學（收斂版）**：通過 topic 准入 + `MAX_PAYLOAD`/`MAX_FIELDS` + **FR-601 欄位約束**的訊息照單全收，未註冊 signal 照收（落庫與否不依賴 registry；但**對外曝露依賴 registry**，見 FR-605）。**驗收**：pub 規則 #1–#4 訊息→窄表落列；違規 topic/oversize/regex/超長 value_text→拒收+metric；device-service 既有測試不因抽取共享套件而變紅。
...
| R7 | **TimescaleDB 版本漂移**（compose `latest-pg15`，壓縮語法依版本而異 → migration 019 不可重現）| 中 | **pin 具體版本 tag**（FR-602 前置）+ migration smoke test（套 019 斷言 hypertable/壓縮 policy 存在）|
