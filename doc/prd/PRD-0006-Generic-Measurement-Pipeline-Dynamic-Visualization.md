...
- **FR-601 窄表 `signal_measurements`**〔P1,M〕：`public.signal_measurements(time TIMESTAMPTZ, device_id TEXT, signal_name TEXT, value_num DOUBLE PRECISION, value_bool BOOLEAN, value_text TEXT)`，CHECK「三個 value 欄**恰一非 NULL**」；hypertable（chunk interval 可調，入 tunable-parameters）；索引 `(device_id, signal_name, time DESC)`。
  - **欄位約束（review P1：cardinality / 外洩防護；DB CHECK + ingest 拒收雙層）**：`signal_name` 須符 `^[a-zA-Z0-9_-]{1,64}$`（**沿用 FR-322 既有 regex**；DB CHECK + ingest 驗證，不符→整列拒收+metric）；`signal_name` 禁用保留字——**清單＝窄表全部欄位名**（`time, device_id, signal_name, value_num, value_bool, value_text`），由共享 parser 套件以具名常數 `RESERVED_SIGNAL_NAMES` 維護；**同源機制（owner review）**：PostgreSQL migration 無法 import Python 常數——一致性由 **drift 測試**保證（整合測試讀 DB CHECK 定義與 `RESERVED_SIGNAL_NAMES` 比對，**同 audit_repo `EVENT_TYPES`↔migration 014 CHECK 的既有 drift-test 樣式**），日後加欄位兩處同改、測試紅即擋（security MED-4）；`value_text` 上限 **256 字元**（DB CHECK `char_length(value_text)<=256`；ingest 超限→**拒收**該欄+metric，不截斷——截斷=靜默失真）。**cardinality**：單訊息沿用 `MAX_FIELDS=64`；per-device 相異 signal 數為軟限（Grafana 計數監看、>64 告警），不在熱路徑硬擋。
...
| R10 | **合法格式洪流 DoS**（broker 無認證 R-003 下高速發布合法 shape 訊息；**輪換 device_id 可繞過 per-device 限速**）| 高 | FR-603 **三層限速**（per-device + 全域硬封頂 `MAX_ROWS_TOTAL_PER_SEC` + per-prefix 監看）；**mosquitto ACL/auth 為 production gate**（根治偽造來源；R-003）|
