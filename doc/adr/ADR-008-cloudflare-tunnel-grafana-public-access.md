...
| 方案 | 改動範圍 | 風險 | 適合期 |
|------|---------|------|-------|
| A. 走既有 GW-HOST Nginx + Let's Encrypt | 改 nginx 路由、申請 cert、開 inbound port | 影響 synaiq-ai.com 主站、TURN-SVC、iptables | 長期正式 |
...
- 不動既有 GW-HOST nginx / Let's Encrypt
- 訪客體驗：瀏覽器即可，無 VPN 安裝
...
- 若轉為長期對外服務，啟動 ADR 評估遷移到方案 A（GW-HOST Nginx）
