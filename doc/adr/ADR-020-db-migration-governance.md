...
EMS 的 DB schema 由 `infra/timescaledb/migrations/` 的**手編號 raw SQL** 管理（`000_*` ~ `018_*`，共 15 支；002 跳號）。現況：
...
**3. 最小可行方案（若不導入完整工具）**
- 加一個 **`schema_migrations` 表**（filename / applied_at），配一支極簡 runner（依序套未記錄者、寫入表）。**保留 000–018 既有 SQL 不重寫歷史**——以 baseline 方式把現有編號標為已套用，自 **019 起**納入新流程。
...
- 不重寫 000–018 歷史 → 遷移成本低、不破壊既有 idempotent 保證。
...
**後續觸發（採用後）**
- 落地選定工具 / 最小 runner + `schema_migrations` 表（baseline 000–018，019 起納管）。
