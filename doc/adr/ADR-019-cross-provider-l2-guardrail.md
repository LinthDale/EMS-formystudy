...
- `GUARDRAIL_PROVIDER=anthropic` 啟用；`GUARDRAIL_API_KEY` 放 Anthropic key（空則**不**沿用 `LLM_API_KEY`，因跨廠商 key 不可共用 → 啟動須 fail-closed 警告 / 拒啟）。
- `guardrail_base_url` 仍走 FR-342 allowlist（加入 `api.anthropic.com`）。

**3. 預算計量沿用 FR-340**
- L2 成本仍進 `provider='guardrail'` ledger row、獨立月預算、fail-closed —— 跨廠商不改 FR-340 機制，只是 g_model / pricing 換成 Anthropic 定價（`llm_pricing_json` 或內建表補 claude-haiku 定價）。
...
**阻塞 / 後續觸發**
- **阻塞**：需使用者提供 Anthropic API key 才能做實機 E2E（在此之前維持 Proposed）。
