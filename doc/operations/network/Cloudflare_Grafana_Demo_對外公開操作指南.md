...
### 0.1 這次不要先動 DGX Nginx

你現有的 YIYI-Global-Gateway / DGX Nginx 是正式入口架構，牽涉：
...
| DGX Gateway 主機 | `http://<EMS_TAILSCALE_IP>:3000` | 可行，但這次暫不把 Nginx 納入 |
...
```text
DGX：正式對外 IP / gateway / Nginx / Coturn / AdGuard
EMS：Grafana 實際運行主機，位於 DESKTOP-G19V1B5 / WSL Ubuntu
DGX ↔ EMS：兩台用 Tailscale 串接
```
...
但那是第二階段。做之前要先在 DGX 上驗證：

```bash
curl -I http://<EMS_TAILSCALE_IP>:3000/login
```

這次先不走 DGX，避免影響現有 YIYI-Global-Gateway。
...
| DGX Gateway 主機 | `http://<EMS_TAILSCALE_IP>:3000` |
...
`cloudflared` 是 Cloudflare Tunnel 的小程式。它可以跑在 EMS 主機、Windows 主機、WSL Ubuntu、DGX，或任何一台能連到 EMS Grafana 的機器。
